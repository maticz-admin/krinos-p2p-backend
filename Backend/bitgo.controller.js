...
export const depositwebhook = async (req, res) => {
    try {
        ...
        if (reqBody?.state === 'confirmed' && reqBody?.transferType === "receive" && reqBody?.value > 0) {
            ...
            let walletPhrase = userWalletData?.bitgo_passphrase;
...
//     try {
//         console.log("internal transfer", walletid, symbol, amount, recipientAddress, walletphrase);

//         const wallet = await bitgo.coin(symbol).wallets().get({ id: walletid });
//         console.log(`Wallet Found: ${wallet.label()} (${wallet.id()})`);

//         const transfer = await wallet.sendMany({
//             amount: parseInt(amount),  // Ensure it's in satoshis
//             address: recipientAddress,
//             walletPassphrase: walletphrase, // ✅ Use real passphrase
//             type: "transfer",
//             isWalletAddress: internal === true
//         });
...
export async function internalTransfer(walletid, symbol, amount, recipientAddress, walletphrase, internal) {
    try {
        ...
        const transfer = await wallet.sendMany({
            ...
            walletPassphrase: walletphrase,
            type: "transfer",
            gasPayer: 'enterprise', // For token transfers on chains like Polygon
            isWalletAddress: internal === true
        });
        ...
        console.error("❌ Error in Internal Transfer:", error?.message || error);
        throw error;
    }
}
